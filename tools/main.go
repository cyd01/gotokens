...
// Convert secondes to human readable (based on 8h or 24h day)
func SecToDay(sec int64, day int) string {
	st := ""

	if sec >= int64(day*3600) {
		st = strconv.FormatInt(int64(sec/int64(day*3600)), 10) + "d"
		sec = sec % int64(day*3600)
	}
	if sec >= int64(3600) {
		if len(st) > 0 {
			st = st + " "
		}
		st = st + strconv.FormatInt(int64(sec/int64(3600)), 10) + "h"
		sec = sec % int64(3600)
	}
	if sec >= int64(60) {
		if len(st) > 0 {
		...
		st = st + strconv.FormatInt(int64(sec/int64(60)), 10) + "m"
		sec = sec % int64(60)
	}
	if sec > 0 {
		st = st + strconv.FormatInt(sec, 10) + "s"
	}
	return st
}
...
// Iterate in an array to check if contains a specific item
